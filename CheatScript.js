...
(() => {
  ...
  const proposePlans = (state, memory) => {
    ...
    const seas = listTiles(state, (t) => t.terrain === 'sea');
    const improvables = listTiles(
      state,
      (t) => (t.facility === 'farm' || t.facility === 'factory' || t.facility === 'oilRig') && !t.enhanced
    );
    ...
    for (let i = 0; i < room; i++) {
      ...
      if (monumentTiles.length > 0 && simulatedMoney < 5000) {
      ...
      if (monumentTiles.length === 0 && simulatedMoney >= MONUMENT_COST) {
        const site = chooseBuildSite(plainEmptyTiles, usedPositions);
        if (site) {
      ...
      if (state.monsters.length > 0 && guns > 0 && simulatedMoney >= 120) {
      ...
      if ((simulatedMoney < 120 || foodSurplus) && state.food >= 40) {
        const amount = foodSurplus ? Math.max(1, Math.min(30, Math.floor(state.food / 100))) : Math.min(5, Math.floor(state.food / 20)) || 1;
      ...
      if (simulatedMoney > 15000 && improvables.length > 0) {
        const p = chooseBuildSite(improvables, usedPositions);
        if (p) {
          plans.push({ action: 'enhanceFacility', x: p.x, y: p.y });
